...
- [ ] **Mirino veritiero**: oggi il mirino è un punto fisso al centro schermo mentre il proiettile parte dalla bocca del cannone — parallasse a corta distanza — e la mira oltre gittata viene schiacciata sul piano acqua (`aim.y = 0` in `boat_cannon.gd::_aim_point`), quindi su bersagli in quota l'impatto non coincide mai. Il mirino (`scenes/hud/crosshair.gd`) deve mostrare il **punto d'impatto reale previsto**: proiettare la traiettoria balistica dal muzzle (gravità 18, `projectile_speed` del `.tres`) e disegnare lì il marker di caduta. Redesign grafico: punto di mira + marker d'impatto + anello di ricarica, leggibile su cielo e mare; fuori gittata il marker lo dice (colore/forma).
- [ ] **Camera: orbita col tasto destro** (nuova azione input, es. `camera_orbit`): il mouse muove la visuale solo a destro premuto; al rilascio, ritorno morbido dietro la poppa (riusa `orbit_return_delay`/`orbit_return_smoothing` di `chase_camera.gd`). **Ritorno sospeso in combattimento**: finestra di combattimento (~8 s, `@export`) rinnovata da sparo, colpo incassato o destro premuto — finché è aperta la camera resta dove la lasci.
- [ ] **Camera più alta in orizzontale**: la vista orizzontale è troppo bassa — ritoccare `pitch_min_deg` (-10°), `min_height` (1.2), `height`/`look_height`. Già tutti `@export`: taratura in playtest con Stefano.
- [ ] **Fumo scafo danneggiato**: nessun feedback progressivo oggi (solo flash barra + shake). Particelle sulla barca agganciate a `hull_changed`: fumo grigio sotto 1/2, fumo nero + scintille sotto 1/3 (soglie `@export`). Pattern `CPUParticles3D` come lo splash di `boat.gd`. Se costa poco, riusarlo anche sulle navi IA (hanno `Damageable`).
- [ ] **Gli eventi casuali fermano il tempo**: oggi `EventDirector` blocca solo la barca e il mondo continua a girare — **il timer delle consegne scorre mentre leggi l'evento** (bug di fatto). Allineare al pattern del pannello inventario: `get_tree().paused = true` + `process_mode` corretto sul CanvasLayer dell'evento.
