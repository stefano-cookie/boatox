...
- [~] Audio: mare, motore, UI, un tema musicale *(anticipato — vedi §3 spunti)*
  ...
  - *Fatto (audio procedurale, i kit CC0 non sono scaricabili da script come i modelli/icone): `Audio` autoload ([autoload/audio.gd](autoload/audio.gd)) crea i bus Master/Music/Ambient/Sfx e genera gli stream con `AudioSynth` ([scripts/audio_synth.gd](scripts/audio_synth.gd)). Mare e motore sono loop continui pilotati dalla barca (`Audio.update_sea`/`update_engine`); gli SFX (urto, vendita cha-ching, boa/boa-blu, pesce, radar, allarme, tick UI) sono agganciati ai segnali di `GameState` (aggiunti `buoy_collected`/`fish_caught`/`cargo_sold`/`radar_pinged`). Mare a due strati d'onde (calmo + burrasca con swell) in crossfade sull'agitazione: cambia timbro, non solo volume. Motore = loop a cicli interi esatti (niente crossfade/rumore → via i ticchettii in accelerazione). Musica = playlist di 4 brani generati (Frutiger Aero: maggiore, arioso, campanellini) a rotazione con pause stile Minecraft, costruiti in un thread per non far scattare l'avvio. Volumi Musica/Effetti dal menu pausa (due slider, salvati in `user://settings.cfg`).* **Da verificare in gioco**: mix dei volumi, timbro motore/mare, gusto dei brani, e se il tick su ogni avviso stanca. **Restano per M4**: eventuali tracce registrate al posto di quelle procedurali + le altre impostazioni (master, sensibilità mouse) accanto ai volumi già fatti.
